...
By default, `yardstick` provides three metrics. `rmse` is [Root-Mean-Square
Error](https://en.wikipedia.org/wiki/Root-mean-square_deviation), `mae` is [Mean
Absolute Error](<https://en.wikipedia.org/wiki/Mean_absolute_error>) `rsq` is $R^2$, our
preferred metric for determining model fit.
